(function () {
  ...
  // Pro attributes (ignored if isPro === false)
  const proStyle = script?.getAttribute("data-style") || "popup"; // "popup" or "inline"
  const proLimit = parseInt(script?.getAttribute("data-limit") || "5", 10);
  const proCategory = script?.getAttribute("data-category") || null;
  const proPosition = script?.getAttribute("data-position") || "bottom-right";
  const proTarget = script?.getAttribute("data-target") || null;
  ...
  async function fetchEntries() {
    try {
      // Use proLimit if Pro, otherwise default to 5
  ...
  function injectStyles(currentStyle, currentPosition) {
  ...
  function renderInline(productName) {
    const selector = proTarget;

    function doRender() {
      const targetEl = selector ? document.querySelector(selector) : null;

      if (!targetEl) return false;
      if (targetEl.dataset.pushlogLoaded) return true; // Guard gegen Duplicates
    ...
    // Fallback: MutationObserver wartet bis Target im DOM ist
    ...
    // Safety timeout: nach 10s aufgeben
  ...
  function createPopup(productName) {
    ...
    if (position === "bottom-left") {
      popup.classList.add("bottom-left");
    }
    const footerClass = isPro ? "pushlog-popup-footer hidden" : "pushlog-popup-footer";
    popup.innerHTML = `
  ...
  async function init() {
    ...
    // Apply Pro attributes only if isPro is true
    if (isPro) {
      style = proStyle;
      position = proPosition;
      limit = Math.min(proLimit, 10);
      category = proCategory;
    }

    injectStyles(style, position);
